...
export default function HairstyleResults() {
  ...
  const hairTextureParam = searchParams.get('hair_texture');
  ...
  const styles = {
    bob: {
      ...
      best_variations: [
        ...
        {
          text: "Curly bob (showcases natural curl pattern with shape and definition)",
          pinterestLink: "https://www.pinterest.com/search/pins/?q=curly%20bob%20haircut"
        },
        {
          text: "Curly shag bob (layers enhance curls and reduce bulk)",
          pinterestLink: "https://www.pinterest.com/search/pins/?q=curly%20shag%20bob%20haircut"
        },
        {
          text: "Tapered curly bob (shorter at the nape, longer on top for curl definition)",
          pinterestLink: "https://www.pinterest.com/search/pins/?q=tapered%20curly%20bob%20haircut"
        }
      ],
      styling_tips: [
        ...
        "For curly hair: Apply styling products to soaking wet hair",
        "For curly hair: 'Scrunch' hair with a microfiber towel to enhance curl pattern",
        "For curly hair: Use a diffuser attachment when blow-drying to maintain curl definition",
        "For curly hair: Refresh second-day curls with water and leave-in conditioner mix",
        "For curly hair: Try finger-coiling for more defined curls"
      ],
      products: {
        budget: [
          {
          ...
          },
          {
            ...
            price: "$8.00",
            benefits: "Lightweight formula, adds bounce and definition to curls, enhances natural curl pattern",
            cons: "May need additional product for very tight curls, limited availability in some stores",
            link: "https://www.ulta.com/p/killer-curls-rinse-pimprod2025457"
          },
          {
        ...
        premium: [
          ...
          },
          {
            ...
            link: "https://www.ouidad.com/products/advanced-climate-control-heat-and-humidity-extra-hold-gel?variant=42617150963970#swiperSlide-31767274586370"
          }
        ]
      },
      celebrity_inspiration: ["Victoria Beckham", "Zendaya", "Hailey Bieber", "Lisa", "Kaia Gerber"]
    },
    pixie: {
      ...
      celebrity_inspiration: ["Zoë Kravitz", "Cara Delevingne", "Halle Berry", "Emma Watson", "Rihanna"]
    },
    long_layers: {
      ...
      celebrity_inspiration: ["Jennifer Aniston", "Sofia Vergara", "Blake Lively", "Priyanka Chopra", "Selena Gomez"]
    },
    curly_natural: {
      ...
      celebrity_inspiration: ["Zendaya", "Sandra Oh", "Yara Shahidi", "Taylor Swift", "Shay Mitchell"]
    },
    updo: {
  ...
  useEffect(() => {
    ...
    if (styleParam && styleParam in styles) {
      let styleResult = styles[styleParam as keyof typeof styles];
      
      // If user has curly hair and is viewing bob style, highlight curly recommendations
      if (styleParam === 'bob' && hairTextureParam === 'curly') {
        // Create a copy to avoid modifying the original
        styleResult = {
          ...styleResult,
          description: styleResult.description + " Your natural curls will look beautiful with these styles, which are specifically chosen to enhance your curl pattern.",
          // Reorder the variations to show curly options first
          best_variations: [
            // Show curly options first
            ...styleResult.best_variations.filter(v => v.text.toLowerCase().includes('curly')),
            // Then show other options
            ...styleResult.best_variations.filter(v => !v.text.toLowerCase().includes('curly'))
          ],
          // Filter styling tips to prioritize curly-specific tips
          styling_tips: [
            // Show curly-specific tips first
            ...styleResult.styling_tips.filter(tip => tip.includes('curly') || tip.includes('For curly hair')),
            // Then show other general tips that might be useful
            ...styleResult.styling_tips.filter(tip => !tip.includes('curly') && !tip.includes('For curly hair'))
          ]
        };
      }
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl shadow-xl overflow-hidden mb-8">
        <div className={`py-8 px-6 text-center ${
          ...
          {/* Special note for curly-haired users getting bob result */}
          {styleParam === 'bob' && hairTextureParam === 'curly' && (
            <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-md max-w-2xl mx-auto">
              <h3 className="font-bold text-green-800 mb-1">Special Note for Your Curly Hair</h3>
              <p className="text-green-700 text-sm">
                Curly bobs require special consideration to work with your natural texture. Look for stylists who specialize in curly hair and consider a "dry cutting" technique. A skilled stylist will work with your curl pattern to create a shape that enhances your natural texture while maintaining the bob silhouette.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-8">
                ...
                {result.best_variations.map((variation: any, index: number) => (
                  <li key={index} className="bg-white border border-gray-200 rounded-lg p-4 transition hover:shadow-md">
                    <div className="flex justify-between items-center">
                      ...
                      >
                        <FaPinterest className="w-4 h-4 mr-1" />
                        See Examples
                      </a>
                    </div>
                  </li>
          ...
          {activeTab === 'styling' && (
            <div>
              ...
              {/* Special curly bob styling guide */}
              {styleParam === 'bob' && hairTextureParam === 'curly' && (
                <div className="bg-green-50 p-4 rounded-md border border-green-100 mt-6 mb-6">
                  <h3 className="font-bold text-green-800 mb-2">Curly Bob Styling Guide</h3>
                  <p className="text-green-700 mb-2">
                    Curly bobs look amazing when maintained properly. Here's a quick routine for your curly bob:
                  </p>
                  <ol className="list-decimal ml-5 text-green-700 space-y-1">
                    <li>Start with sulfate-free shampoo and moisture-rich conditioner</li>
                    <li>Apply leave-in conditioner while hair is soaking wet</li>
                    <li>Add gel or curl cream with "praying hands" method</li>
                    <li>Scrunch with a microfiber towel to remove excess water</li>
                    <li>Avoid touching hair while it dries (or use a diffuser on low heat)</li>
                    <li>Once fully dry, scrunch out any crunchiness for soft, defined curls</li>
                  </ol>
                </div>
              )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white p-4 rounded-md border border-gray-200 shadow-sm">
                  <h4 className="font-semibold text-gray-800 mb-3">Celebrities with this style:</h4>
                  <ul className="space-y-3">
                    {result.celebrity_inspiration && result.celebrity_inspiration.map((celeb: string, index: number) => (
                ...
                <div className="bg-white p-4 rounded-md border border-gray-200 shadow-sm">
                  <h4 className="font-semibold text-gray-800 mb-3">Celebrity Social Media:</h4>
                  <ul className="space-y-3">
                    {styleParam === 'bob' && (
                      <>
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Victoria Beckham:</span>
                          <a href="https://www.instagram.com/victoriabeckham/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <FaInstagram className="w-4 h-4 mr-1" />
                            Instagram
                          </a>
                          <a href="https://www.youtube.com/victoriabeckham" target="_blank" rel="noopener noreferrer" className="text-red-600 hover:underline flex items-center">
                        ...
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Hailey Bieber:</span>
                          <a href="https://www.instagram.com/haileybieber/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                            </svg>
                            Instagram
                    ...
                    {styleParam === 'pixie' && (
                      <>
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Zoë Kravitz:</span>
                          <a href="https://www.instagram.com/zoeisabellakravitz/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                            </svg>
                            Instagram
                        ...
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Cara Delevingne:</span>
                          <a href="https://www.instagram.com/caradelevingne/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                            </svg>
                            Instagram
                          </a>
                          <a href="https://www.youtube.com/c/CaraDelevingne" target="_blank" rel="noopener noreferrer" className="text-red-600 hover:underline flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Emma Watson:</span>
                          <a href="https://www.instagram.com/emmawatson/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                            </svg>
                            Instagram
                    ...
                    {styleParam === 'long_layers' && (
                      <>
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Jennifer Aniston:</span>
                          <a href="https://www.instagram.com/jenniferaniston/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                            </svg>
                            Instagram
                        ...
                        <li className="flex items-center">
                          <span className="font-medium mr-2">Priyanka Chopra:</span>
                          <a href="https://www.instagram.com/priyankachopra/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-3 flex items-center">
                            <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                            </svg>
                            Instagram
